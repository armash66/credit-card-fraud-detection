...
col1.metric(
    "Isolation Forest Rate",
    f"{df['is_if_anomaly'].mean() * 100:.2f}%"
)

col2.metric(
    "LOF Rate",
    f"{df['is_lof_anomaly'].mean() * 100:.2f}%"
)

col3.metric(
    "PCA Rate",
    f"{df['is_pca_anomaly'].mean() * 100:.2f}%"
)

st.caption(
    "Each model is calibrated to ~1% anomalies for fair comparison. "
...
st.caption(
    "Higher agreement → higher confidence of suspicious behavior."
)
...
st.area_chart(
    df["risk_score"].sort_values().reset_index(drop=True)
)

st.caption(
    "Right tail represents high-risk transactions."
)
...
st.caption(
    "High confidence alerts are best candidates for manual review."
)
...
# =========================
st.success(
    "✔ Multiple models agree on a small, high-risk subset — "
    "this indicates strong anomaly detection reliability."
)
