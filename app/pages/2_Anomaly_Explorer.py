...
required_cols = [
    "transaction_id", "risk_score", "amount", "hour",
    "error_present", "is_if_anomaly", "is_lof_anomaly", "is_pca_anomaly",
]
...
risk_q = st.sidebar.slider("Risk Threshold (Top %)", min_value=0.90, max_value=0.995, value=0.99, step=0.005)
agreement = st.sidebar.selectbox("Model Agreement", ["Any model", "At least 2 models", "All 3 models"])
max_rows = st.sidebar.slider("Rows to display", min_value=50, max_value=300, value=100, step=50)

risk_threshold = df["risk_score"].quantile(risk_q)
df_f = df[df["risk_score"] >= risk_threshold].copy()

if agreement == "All 3 models":
    df_f = df_f[(df_f["is_if_anomaly"] == 1) & (df_f["is_lof_anomaly"] == 1) & (df_f["is_pca_anomaly"] == 1)]
elif agreement == "At least 2 models":
    df_f["model_votes"] = df_f["is_if_anomaly"] + df_f["is_lof_anomaly"] + df_f["is_pca_anomaly"]
    df_f = df_f[df_f["model_votes"] >= 2]

df_f = df_f.sort_values("risk_score", ascending=False).head(max_rows).copy()
...
def explain_row(row):
    ...
    if row["error_present"] == 1:
        reasons.append("Transaction error detected")
    model_votes = row["is_if_anomaly"] + row["is_lof_anomaly"] + row["is_pca_anomaly"]
    if model_votes == 3:
...
def generate_explanations(df_chunk):
    ...
    df_chunk["summary"] = df_chunk["why_flagged"].apply(lambda x: " | ".join(x))
    return df_chunk
...
display_cols = ["transaction_id", "amount", "hour", "risk_level", "risk_score", "summary"]
...
col2.metric("Alert Rate", f"{(len(df_f) / len(df)) * 100:.2f}%")

st.info("Explainable anomaly detection active.")
